...
SELECT ord.id, c.first_name, c.last_name,  c.email
FROM `orders` as ord 
...
SELECT  MONTH(date_order), COUNT(`id`) 
FROM `orders` 
...
-- Folosesc abrevieri pentru a usura scrierea query-ului
SELECT op.`id_product`,COUNT(op.`id_order`) as nr_comenzi, p.`name` FROM `orders_products` as op
